...
// Avrundat årsspann i stället för en falsk decimal — samma logik som webbens
// ganttSpann(). Normalfallet kan diffa ~30 %, så "3,8 år" ljuger om precisionen.
function ganttSpann(m) {
  if (m < 18) return `${m} mån`;
  const y = m / 12;
  let lo = Math.round(y * 0.8);
  let hi = Math.round(y * 1.25);
  if (hi - lo < 1) hi = lo + 1;
  return `${lo}–${hi} år`;
}
...
function mkShadow() {
  // Mjuk, låg skugga som på webben (0 2px 12px rgba(26,35,69,0.06)).
  return { type: 'outer', color: '1A2744', blur: 11, offset: 2.5, angle: 90, opacity: 0.09 };
}

// Gemensam hörnradie — samma rundade formspråk som HTML-korten (8–10px).
const R = 0.12;

// Mjukt, rundat kort med hårfin kant och låg skugga — grundformen på webben.
function softCard(slide, { x, y, w, h, fill = PALETT.white, line = PALETT.border, shadow = true, radius = R }) {
  slide.addShape('roundRect', {
    x, y, w, h, rectRadius: radius,
    fill: { color: fill }, line: { color: line, width: 1 },
    ...(shadow ? { shadow: mkShadow() } : {}),
  });
}
...
function addCallout(slide, callout, { x, y, w, h }) {
  ...
  slide.addShape('roundRect', {
    x: x + 0.12, y: y + 0.14, w: 0.07, h: h - 0.28, rectRadius: 0.03,
...
// De fyra spåren med sina färger — samma som webbens spårkort.
const SPAR_BAND = [
  { b: 'A', namn: 'Hyra in', farg: '0891B2' },
  { b: 'B', namn: 'Bygga om', farg: '059669' },
  { b: 'C', namn: 'Tillbyggnad', farg: 'D97706' },
  { b: 'D', namn: 'Nybyggnad', farg: 'C0392B' },
];

function layoutTitel(slide, s) {
  slide.background = { color: PALETT.navy };
  // Guldregel i vänsterkanten som fäster titelblocket.
  slide.addShape('rect', { x: 0, y: 0, w: 0.16, h: H, fill: { color: PALETT.gold }, line: { color: PALETT.gold } });

  slide.addText('Lejonfastigheter · Lokalförsörjning', {
    x: 0.7, y: 0.6, w: W - 1.4, h: 0.4,
    fontSize: 11, fontFace: 'Calibri',
    color: PALETT.goldLight, bold: true, charSpacing: 3,
  });
  slide.addText(s.titel, {
    x: 0.7, y: 1.65, w: W - 1.4, h: 1.7,
    fontSize: 58, fontFace: 'Georgia',
    color: PALETT.white, bold: true, valign: 'top', fit: 'shrink',
  });
  slide.addText(s.undertitel, {
    x: 0.7, y: 3.35, w: W - 2.4, h: 0.55,
    fontSize: 18, fontFace: 'Calibri',
    color: PALETT.goldLight, valign: 'top',
  });
  slide.addText(s.ingress, {
    x: 0.7, y: 4.0, w: 8.7, h: 1.2,
    fontSize: 14, fontFace: 'Calibri',
    color: 'C8CFD8', valign: 'top',
  });

  // Färgstark spår-rad i botten — fyller ytan med de fyra vägarna materialet handlar om.
  const bandY = 5.55;
  const bw = 2.85;
  const bgap = 0.28;
  const startX = 0.7;
  SPAR_BAND.forEach((sp, i) => {
    const x = startX + i * (bw + bgap);
    slide.addShape('roundRect', { x, y: bandY, w: bw, h: 0.9, rectRadius: 0.1, fill: { color: '223154' }, line: { color: '2E3F63' } });
    const d = 0.56;
    slide.addShape('ellipse', { x: x + 0.18, y: bandY + (0.9 - d) / 2, w: d, h: d, fill: { color: sp.farg }, line: { color: sp.farg } });
    slide.addText(sp.b, {
      x: x + 0.18, y: bandY + (0.9 - d) / 2, w: d, h: d, margin: 0,
      fontSize: 22, fontFace: 'Georgia', bold: true,
      color: PALETT.white, align: 'center', valign: 'middle',
    });
    slide.addText(sp.namn, {
      x: x + 0.9, y: bandY, w: bw - 1.0, h: 0.9, margin: 0,
  ...
  if (s.tagline) {
    slide.addText(s.tagline, {
      x: 0.7, y: H - 0.55, w: W - 1, h: 0.35,
      fontSize: 10, fontFace: 'Calibri',
      color: PALETT.goldLight, charSpacing: 3, italic: true,
    });
  }
}
...
function layoutKortGrid(slide, s) {
  ...
  kort.forEach((k, i) => {
    ...
    softCard(slide, { x, y: kortY, w: kortW, h: kortH });
    // Webbens kortmotiv: tunn rundad accent i överkant
    const kant = k.farg || PALETT.gold;
    slide.addShape('roundRect', {
      x: x + 0.12, y: kortY + 0.02, w: kortW - 0.24, h: 0.09, rectRadius: 0.04,
...
function layoutInnehall(slide, s) {
  slide.background = { color: PALETT.parchment };
  commonHeader(slide);
  addTitel(slide, s, { y: 0.62, h: 0.9 });

  const bandY = 1.95;
  const bandH = BOTTEN - bandY;

  if (Array.isArray(s.stycken) && s.stycken.length) {
    // Brödtext i ett samlat kort med guldkant och kapad radlängd — läser som
    // en ledartext, inte en textvägg över hela sidbredden.
    const cardW = 10.8;
    const cardX = (W - cardW) / 2;
    const textSize = 15;
    const inreBredd = cardW - 1.1;
    const behov = s.stycken.reduce((h, t) => h + hojdFor(t, inreBredd, textSize, 0.3) + 0.14, 0) + 0.5;
    const cardH = Math.min(bandH, Math.max(2.2, behov));
    const cardY = bandY + Math.max(0, (bandH - cardH) / 2);
    softCard(slide, { x: cardX, y: cardY, w: cardW, h: cardH, fill: 'FBF7EC', line: 'E8DFC8' });
    slide.addShape('roundRect', { x: cardX + 0.12, y: cardY + 0.18, w: 0.07, h: cardH - 0.36, rectRadius: 0.03, fill: { color: PALETT.gold }, line: { color: PALETT.gold } });
    slide.addText(
      s.stycken.map(t => ({ text: t, options: { breakLine: true } })),
      {
        x: cardX + 0.55, y: cardY + 0.1, w: inreBredd, h: cardH - 0.2,
        fontSize: textSize, fontFace: 'Calibri',
        color: PALETT.navy, valign: 'middle', paraSpaceAfter: 13, lineSpacingMultiple: 1.2, fit: 'shrink',
      }
    );
    ...
    const cardW = spalter === 2 ? W - 1.4 : 10.2;
    const cardX = (W - cardW) / 2;
    const cardH = Math.min(bandH, Math.max(2.2, bandH - 0.4));
    const cardY = bandY + Math.max(0, (bandH - cardH) / 2);
    softCard(slide, { x: cardX, y: cardY, w: cardW, h: cardH, fill: 'FBF7EC', line: 'E8DFC8' });
    slide.addShape('roundRect', { x: cardX + 0.12, y: cardY + 0.18, w: 0.07, h: cardH - 0.36, rectRadius: 0.03, fill: { color: PALETT.gold }, line: { color: PALETT.gold } });
    const spaltW = (cardW - 1.0 - (spalter - 1) * 0.5) / spalter;
    for (let sp = 0; sp < spalter; sp++) {
      const del = s.punkter.slice(sp * perSpalt, (sp + 1) * perSpalt);
      slide.addText(
        del.map(t => ({ text: t, options: { bullet: { code: '25CF', indent: 14 }, breakLine: true } })),
        {
          x: cardX + 0.55 + sp * (spaltW + 0.5), y: cardY + 0.2, w: spaltW, h: cardH - 0.4,
          fontSize: spalter === 2 ? 12.5 : 14, fontFace: 'Calibri',
          color: PALETT.navy, valign: 'middle', paraSpaceAfter: 10, lineSpacingMultiple: 1.15, fit: 'shrink',
        }
      );
    }
  }
...
function layoutGantt(slide, s) {
  ...
  rader.forEach((rad, i) => {
    ...
    // Totaltid efter stapeln — avrundat årsspann, inte en falsk decimal.
    // Långa staplar (nära sidkanten) får etiketten inne i stapeln, vit och
    // högerställd, så den inte hamnar utanför sidytan.
    const tid = ganttSpann(rad.totalManader);
    const bx = mx(rad.totalManader);
    const inne = bx + 1.18 > W - 0.2;
    slide.addText(nbUnit(tid), {
      x: inne ? bx - 1.18 : bx + 0.08, y: y + 0.02, w: 1.1, h: 0.4, margin: 0,
      fontSize: 10, fontFace: 'Georgia', bold: true,
      color: inne ? PALETT.white : PALETT.navy,
      align: inne ? 'right' : 'left', valign: 'middle',
    });
  });
...
function layoutSparKort(slide, s) {
  ...
  kort.forEach((k, i) => {
    const x = 0.7 + i * (kortW + gap);
    softCard(slide, { x, y: kortY, w: kortW, h: kortH, fill: PALETT.parchment });
    ...
    slide.addShape('roundRect', {
      x: x + 0.18, y: kortY + kortH - 0.95, w: kortW - 0.36, h: 0.8, rectRadius: 0.07,
...
function layoutSteg(slide, s) {
  ...
  const etikettH = 0.3;
  const gap = 0.14;
  const nrX = 0.95;       // gold serif-siffra i egen kolumn, som webben
  const textX = 1.7;
  const textBredd = 8.4;  // kapad radlängd
  const textSize = 10;
  ...
  const hojder = grupper.map(g => (g.steg || []).map(st => {
    ...
    return Math.max(0.66, 0.4 + n * 0.24);
  }));
  const summa = hojder.flat().reduce((a, b) => a + b, 0)
    + grupper.length * (etikettH + 0.08)
...
function layoutPunktKort(slide, s) {
  ...
  let botten = BOTTEN;
  const calloutH = s.callout ? calloutHojd(s.callout) : 0;
  if (s.callout) botten -= calloutH + 0.35;
  const bandH = botten - bandY;

  // Höjd efter innehåll (rubrikrad + punkter) så korten inte blir tomma lådor.
  const inreBredd = kortW - 0.6;
  const behov = kort.map(k => {
    const punktLinjer = (k.punkter || []).reduce((n, t) => n + rader(t, inreBredd - 0.25, punktSize), 0);
    return 0.95 + punktLinjer * (punktSize / 72) * 1.35 + (k.punkter || []).length * 0.1 + 0.3;
  });
  const kortH = Math.min(bandH, Math.max(2.4, ...behov));
  const kortY = bandY + Math.max(0, (bandH - kortH) / 2);

  kort.forEach((k, i) => {
    const x = 0.7 + i * (kortW + gap);
    const chip = CHIP[k.chipFarg] || CHIP.lag;
    softCard(slide, { x, y: kortY, w: kortW, h: kortH });
    // Tunn färgad toppaccent (blå för lag, grön för kommun) — som webbens kort.
    slide.addShape('roundRect', {
      x: x + 0.12, y: kortY + 0.02, w: kortW - 0.24, h: 0.09, rectRadius: 0.04,
    ...
    let rubrikX = x + 0.35;
    let rubrikW = kortW - 0.65;
    if (k.chip) {
      const chipW = 0.34 + k.chip.length * 0.088;
      slide.addShape('roundRect', {
        x: x + 0.35, y: kortY + 0.32, w: chipW, h: 0.34, rectRadius: 0.17,
        fill: { color: chip.fill }, line: { color: chip.text, width: 0.75 },
      });
      slide.addText(k.chip.toUpperCase(), {
        x: x + 0.35, y: kortY + 0.32, w: chipW, h: 0.34,
        fontSize: 9, fontFace: 'Calibri', bold: true, charSpacing: 1.5,
        color: chip.text, align: 'center', valign: 'middle',
      });
      rubrikX = x + 0.35 + chipW + 0.14;
      rubrikW = kortW - chipW - 0.85;
    }
    if (k.rubrik) {
      slide.addText(k.rubrik, {
        x: rubrikX, y: kortY + 0.3, w: rubrikW, h: 0.4,
        fontSize: 13.5, fontFace: 'Georgia',
        color: PALETT.navy, bold: true, valign: 'middle', fit: 'shrink',
      });
    }
    slide.addText(
      (k.punkter || []).map(t => ({ text: t, options: { bullet: { code: '25CF', indent: 14 }, breakLine: true } })),
      {
        x: x + 0.35, y: kortY + 0.95, w: kortW - 0.65, h: kortH - 1.15,
        fontSize: punktSize, fontFace: 'Calibri',
        color: PALETT.navyMid, valign: 'top', paraSpaceAfter: 9, lineSpacingMultiple: 1.05, fit: 'shrink',
      }
    );
  });

  if (s.callout) {
    addCallout(slide, s.callout, { x: 0.7, y: kortY + kortH + 0.35, w: W - 1.4, h: calloutH });
  }
}
