...
function layoutSiffraStor(slide, s) {
  ...
  if (Array.isArray(s.faser) && s.faser.length) {
    ...
    s.faser.forEach((fas, i) => {
      ...
      slide.addShape('ellipse', {
        x: x + 0.14, y: stripY + 0.15, w: 0.13, h: 0.13,
        fill: { color: farg }, line: { color: farg },
      });
      slide.addText(fas.namn, {
        x: x + 0.36, y: stripY + 0.06, w: segW - 0.48, h: 0.3, margin: 0,
        fontSize: 9.5, fontFace: 'Calibri', bold: true,
        color: PALETT.navy, fit: 'shrink',
      });
      slide.addText(nbUnit(fas.tid), {
        x: x + 0.36, y: stripY + 0.37, w: segW - 0.48, h: 0.3, margin: 0,
        fontSize: 11, fontFace: 'Georgia', bold: true,
        color: PALETT.navyMid,
      });
      if (i < s.faser.length - 1) {
        slide.addText('›', {
          x: x + segW - 0.07, y: stripY + 0.16, w: 0.2, h: 0.4, margin: 0,
          fontSize: 14, fontFace: 'Georgia', bold: true,
          color: PALETT.muted, align: 'center',
        });
      }
    });
  }
...
// vit rad med stort guldnummer, rubrik och en rad beskrivning.
function layoutSteg(slide, s) {
  ...
  const radH = 0.5, gap = 0.07, etikettH = 0.26;
  let y = 1.62;
  for (const g of s.grupper || []) {
...
// Två vita kort med chip-rubrik och punktlista (.prov-varfor-kort) och
// valfri slutsats-callout under — samma motiv som webbens "Varför drar
// det ut på tiden?".
const CHIP = {
  lag:    { fill: 'E9EFFD', text: FASFARGER.plan.farg },
  kommun: { fill: 'E6F4EF', text: FASFARGER.projektering.farg },
};
function layoutPunktKort(slide, s) {
  ...
  const kortW = (W - 1.4 - gap * (kort.length - 1)) / kort.length;
  const kortY = 2.0;
  const kortH = s.callout ? 3.55 : 4.4;
  kort.forEach((k, i) => {
    const x = 0.7 + i * (kortW + gap);
    slide.addShape('rect', {
      x, y: kortY, w: kortW, h: kortH,
    ...
    if (k.rubrik) {
      slide.addText(k.rubrik, {
        x: x + 0.3 + chipW + 0.1, y: kortY + 0.25, w: kortW - chipW - 0.7, h: 0.4,
    ...
    slide.addText(
      (k.punkter || []).map(t => ({ text: t, options: { bullet: { code: '25CF' }, breakLine: true } })),
      {
        ...
        fontSize: 11, fontFace: 'Calibri',
        color: PALETT.navyMid, valign: 'top', paraSpaceAfter: 6,
      }
    );
  });

  if (s.callout) {
    addCallout(slide, s.callout, { x: 0.7, y: kortY + kortH + 0.3, w: W - 1.4, h: 0.8 });
  }
}
