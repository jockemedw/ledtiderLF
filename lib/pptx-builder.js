...
function layoutTitel(slide, s) {
  ...
  SPAR_BAND.forEach((sp, i) => {
    ...
    slide.addShape('rect', { x, y: bandY, w: bw, h: 0.9, fill: { color: '223154' }, line: { color: '223154' } });
    slide.addShape('rect', { x, y: bandY, w: 0.62, h: 0.9, fill: { color: sp.farg }, line: { color: sp.farg } });
    slide.addText(sp.b, {
      x, y: bandY, w: 0.62, h: 0.9,
      fontSize: 26, fontFace: 'Georgia', bold: true,
      color: PALETT.white, align: 'center', valign: 'middle',
    });
    slide.addText(sp.namn, {
      x: x + 0.72, y: bandY, w: bw - 0.82, h: 0.9,
...
function layoutInnehall(slide, s) {
  ...
  if (Array.isArray(s.stycken) && s.stycken.length) {
    ...
    slide.addShape('rect', {
      ...
      fill: { color: PALETT.white }, line: { color: PALETT.border }, shadow: mkShadow(),
    });
    slide.addShape('rect', { x: cardX, y: cardY, w: 0.09, h: cardH, fill: { color: PALETT.gold }, line: { color: PALETT.gold } });
    slide.addText(
...
function layoutSteg(slide, s) {
  ...
  const gap = 0.12;
  const textX = 1.75;
  const textBredd = 9.6;   // kapad radlängd — inte hela sidbredden
  const textSize = 10;
  ...
  const hojder = grupper.map(g => (g.steg || []).map(st => {
    ...
    return Math.max(0.62, 0.34 + n * 0.24);
  }));
  ...
  grupper.forEach((g, gi) => {
    if (g.etikett) {
      slide.addText(g.etikett.toUpperCase(), {
        x: 0.72, y, w: W - 1.4, h: etikettH,
        fontSize: 10, fontFace: 'Calibri',
        color: PALETT.gold, bold: true, charSpacing: 3, valign: 'bottom',
      });
      slide.addShape('rect', { x: 0.72, y: y + etikettH, w: 0.6, h: 0.02, fill: { color: PALETT.gold }, line: { color: PALETT.gold } });
    ...
    (g.steg || []).forEach((st, si) => {
      ...
      // Guldkant + numrerad navy-bricka.
      slide.addShape('rect', { x: 0.7, y, w: 0.08, h: radH, fill: { color: PALETT.gold }, line: { color: PALETT.gold } });
      const badge = 0.5;
      slide.addShape('roundRect', {
        x: 0.98, y: y + (radH - badge) / 2, w: badge, h: badge, rectRadius: 0.08,
        fill: { color: PALETT.navy }, line: { color: PALETT.navy },
      });
      slide.addText(String(st.nr), {
        x: 0.98, y: y + (radH - badge) / 2, w: badge, h: badge,
        fontSize: 20, fontFace: 'Georgia',
        color: PALETT.goldLight, bold: true, align: 'center', valign: 'middle',
      });
      slide.addText(st.titel, {
        x: textX, y: y + 0.08, w: textBredd, h: 0.26,
        fontSize: 12.5, fontFace: 'Georgia',
        color: PALETT.navy, bold: true, valign: 'top',
      });
      slide.addText(st.text, {
        x: textX, y: y + 0.34, w: textBredd, h: radH - 0.4,
        fontSize: textSize, fontFace: 'Calibri',
        color: PALETT.navyMid, valign: 'top', fit: 'shrink',
      });
      y += radH + gap;
...
function layoutPunktKort(slide, s) {
  ...
  kort.forEach((k, i) => {
    ...
    // Färgad kant i vänsterkanten (styrs-av-lag blå / kommun grön).
    slide.addShape('rect', { x, y: kortY, w: 0.1, h: kortH, fill: { color: chip.text }, line: { color: chip.text } });
