...
function layoutSiffraStor(slide, s) {
  ...
  // Långa siffror (t.ex. "3–12 mån") måste få plats i navy-rutan
  const siffraLen = String(s.siffra || '').length;
  const siffraSize = siffraLen <= 5 ? 110 : siffraLen <= 8 ? 72 : 54;
  slide.addText(s.siffra, {
    x: 0.7, y: 2.6, w: 5.0, h: 2.4,
    fontSize: siffraSize, fontFace: 'Georgia',
...
function layoutTabell(slide, s) {
  ...
  const ovriga = (total - forsta) / (antalKol - 1);
  slide.addTable(rows, {
    x: 0.7, y: 2.1, w: total,
...
function layoutKortGrid(slide, s) {
  ...
  kort.forEach((k, i) => {
    ...
    slide.addShape('rect', {
      x, y: kortY, w: kortW, h: 0.07,
      fill: { color: PALETT.gold }, line: { color: PALETT.gold }
    });
    const siffraSize = String(k.siffra || '').length <= 4 ? 40 : 26;
    slide.addText(k.siffra, {
      x: x + 0.2, y: kortY + 0.25, w: kortW - 0.4, h: 0.85,
      fontSize: siffraSize, fontFace: 'Georgia',
      color: PALETT.navy, bold: true, valign: 'middle',
    });
  ...
  if (s.callout) {
    const calloutY = kortY + kortH + 0.25;
    slide.addShape('rect', {
      x: 0.7, y: calloutY, w: W - 1.4, h: 0.85,
      fill: { color: PALETT.navy }, line: { color: PALETT.navy }
    });
