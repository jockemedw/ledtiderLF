...
// I Nexts serverbundle pekar __dirname in i .next/server, så filen löses
// från process.cwd() (projektroten, även på Vercel via outputFileTracing-
// Includes i next.config.js); __dirname-varianten täcker CLI och tester.
const HTML_PATH = [
  path.join(process.cwd(), 'lokalforsorjning.html'),
  path.join(__dirname, '..', 'lokalforsorjning.html'),
].find(p => fs.existsSync(p)) || path.join(process.cwd(), 'lokalforsorjning.html');
