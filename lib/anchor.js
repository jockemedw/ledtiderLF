...
export function readAnchorText(element) {
  if (!element) return '';
...
// Markeringsläget träffar det djupaste elementet under muspekaren (t.ex. en
// <strong> inne i ett stycke). Ankare på element utanför COMMENTABLE_SELECTOR
// återskapas inte vid sidladdning, så kommentaren blir föräldralös — snappa
// därför till närmaste kommenterbara element (inklusive elementet självt).
export function snapToCommentable(el) {
  return el?.closest?.(COMMENTABLE_SELECTOR) || el;
}
