...
const GRUPPER = [
  {
    id: 'planprocess',
    namn: 'Planprocess och lov',
    ingress:
      'Hur lång tid kommunens planering och lovgivning tar, och hur mycket som planeras. Det enda område där Kolada mäter något som direkt motsvarar guidens ledtidsargument. Underlaget är Boverkets plan- och bygglovsenkät och avser bostadsplaner.',
  },
  {
    id: 'lokalkostnad',
    namn: 'Lokalkostnad per verksamhet',
    ingress:
      'Kommunens bokförda lokalkostnad enligt Räkenskapssammandraget — i praktiken hyresnotan till Lejonfastigheter sedd från hyresgästens sida. Kostnadsnivå är inte ett betyg.',
  },
  {
    id: 'demografi',
  ...
  {
    id: 'region-fastighet',
    namn: 'Fastighetsnyckeltal, regionen',
    ingress:
      'Kolada har inga fastighetsnyckeltal per kvadratmeter för kommuner — serien finns bara för regioner. Region Östergötland och dess anläggningar är närmaste offentliga benchmark. Vårdfastigheter är inte skolor.',
  },
];
...
const NOT_BOSTADSPLAN =
  'Boverkets plan- och bygglovsenkät avser bostadsplaner, inte planer för skola, förskola, LSS eller idrott.';
...
const KATALOG = [
  // --- Planprocess ---
  { id: 'N07926', kort: 'Mediantid planuppdrag → antagande', grupp: 'planprocess', enhet: 'månader', decimaler: 1, klass: 'styrmatt', riktning: 'lagre_battre', percentil: true, trend: true, liknande: LIKNANDE_OVRIGT, tangerande: true, not: `${NOT_BOSTADSPLAN} Publiceras vartannat år; 2025 års värde speglar planer antagna 2023–2025. ${NOT_44_46}` },
  { id: 'N07927', kort: 'Mediantid samrådsstart → antagande', grupp: 'planprocess', enhet: 'månader', decimaler: 1, klass: 'styrmatt', riktning: 'lagre_battre', percentil: true, trend: true, liknande: LIKNANDE_OVRIGT, tangerande: true, not: `${NOT_BOSTADSPLAN} Skillnaden mot planuppdrag → antagande visar hur mycket tid som ligger före samråd, alltså i utrednings- och programskedet.` },
  ...
  { id: 'N07928', kort: 'Överklagade detaljplaner, senaste två åren', grupp: 'planprocess', enhet: '%', decimaler: 1, klass: 'kontextmatt', riktning: 'ingen', percentil: true, liknande: LIKNANDE_OVRIGT, tangerande: true, not: 'Andelen speglar både planernas karaktär och kommunens planeringsmiljö, inte enbart handläggningen. Därför kontextmått och inte styrmått.' },
  ...
  { id: 'N07929', kort: 'Antagna detaljplaner senaste två åren', grupp: 'planprocess', enhet: 'antal', decimaler: 0, klass: 'kontextmatt', riktning: 'ingen', percentil: false, liknande: LIKNANDE_OVRIGT, tangerande: true, not: 'Absolut antal, inte normerat mot kommunstorlek — därför ingen percentil.' },
  ...
  { id: 'N03187', kort: 'Interna köp och kostnader, gemensamma lokaler', grupp: 'lokalkostnad', enhet: 'kr/inv', decimaler: 0, klass: 'kostnadsmatt', riktning: 'ingen', percentil: true, liknande: LIKNANDE_OVRIGT, not: 'Påverkas direkt av hur internhyresmodellen är konstruerad — svårjämförbart mellan kommuner.' },
...
function avvikelse(varde, referens) {
  ...
  return avrunda(((varde - referens) / referens) * 100, 1);
}
...
// ------------------------------------------------- den Kolada-kollade guiden

const SEKTIONSSTATUS = ['kompletterad', 'delvis', 'ingen'];

/**
 * Sektionerna i lokalforsorjning.html och vad Kolada kan säga om var och en.
 *
 * `kommentar` är obligatorisk på status 'ingen' — den ska säga VARFÖR Kolada
 * inte kan belägga avsnittet, inte bara att det inte kan. Att märka ett avsnitt
 * som okontrollerbart utan att förklara varför är sämre än att inte märka alls.
 */
const KOLLADE_SEKTIONER = [
  {
    id: 'intro',
    status: 'ingen',
    kpi: [],
    kommentar: 'Ingång och innehållsförteckning — inga påståenden att kontrollera.',
  },
  {
    id: 'sammanfattning',
    status: 'delvis',
    kpi: ['N07926'],
    rubrik: 'Vad Kolada kan säga om sammanfattningen',
    kommentar:
      'Av de fyra punkterna kan Kolada bara belägga en komponent: planskedet, som ingår i intervallet 3 månader till 8 år. Att kostnaden låses tidigt går inte att mäta i Kolada, men tidsfördelningen nedan pekar åt samma håll — merparten av planledtiden ligger före samråd.',
  },
  {
    id: 'oversikt',
    status: 'delvis',
    kpi: ['N07926', 'N07927'],
    rubrik: 'Kolada täcker planskedet i Spår D',
    kommentar:
      'Spår A, B och C har ingen motsvarighet i Kolada — det finns ingen offentlig statistik över hur lång tid inhyrning, ombyggnad eller tillbyggnad tar. För Spår D kan planskedet mätas, och Linköpings egen siffra ligger nedan.',
  },
  {
    id: 'jamforelse',
    status: 'delvis',
    kpi: ['N07926', 'N07927'],
    rubrik: 'Planskedets faktiska längd i Linköping',
    kommentar:
      'Tidsjämförelsen bygger på riksgenomsnitt. Kolada ger Linköpings egen mediantid för den del av processen som avser detaljplan. Byggfasen finns inte i Kolada.',
  },
  {
    id: 'per-typ',
    status: 'ingen',
    kpi: [],
    kommentar:
      'Kolada har inga kostnader per kvadratmeter för kommunala verksamhetslokaler — serien finns bara för regioner. Inga investerings- eller entreprenadkostnader alls, och inga byggtider. Avsnittets tabell måste därför fortsatt vila på enskilda projektreferenser.',
  },
  {
    id: 'spar-a',
    status: 'ingen',
    kpi: [],
    kommentar: 'Ingen offentlig statistik över hur lång tid det tar att hyra in en befintlig lokal.',
  },
  {
    id: 'spar-b',
    status: 'ingen',
    kpi: [],
    kommentar: 'Ingen offentlig statistik över ombyggnadstider för kommunala verksamhetslokaler.',
  },
  {
    id: 'spar-c',
    status: 'ingen',
    kpi: [],
    kommentar: 'Ingen offentlig statistik över tillbyggnadstider.',
  },
  {
    id: 'spar-d',
    status: 'kompletterad',
    kpi: ['N07926', 'N07927'],
    rubrik: 'Planskedet i Linköping',
    kommentar:
      'Detta är det enda spår där Kolada mäter en del av ledtiden. Siffrorna avser bostadsplaner — en detaljplan för skola eller omsorg kan ha annan komplexitet — men de är Linköpings egna, till skillnad från riksgenomsnittet i avsnittets text.',
  },
  {
    id: 'moduler',
    status: 'ingen',
    kpi: [],
    kommentar: 'Modulbyggnader och hyresmoduler finns inte som eget mått i Kolada.',
  },
  {
    id: 'provning',
    status: 'kompletterad',
    kpi: ['N07926', 'N07927', 'N07928', 'N07929', 'N07923'],
    rubrik: 'Prövningen i siffror — Linköping mot jämförbara kommuner',
    kommentar:
      'Boverkets plan- och bygglovsenkät publiceras kommun för kommun i Kolada. Det är samma underlag som ligger bakom SKR:s öppna jämförelser, men nedbrytbart. Notera att andelen överklagade planer är ett kontextmått, inte ett styrmått: den speglar planernas karaktär minst lika mycket som handläggningen.',
  },
  {
    id: 'kontext',
    status: 'kompletterad',
    kpi: ['N07926', 'N07927', 'U00810', 'N07001'],
    rubrik: 'Den nationella bilden, nedbruten på Linköping',
    kommentar:
      'Två saker syns. Planledtiden är längre än hos jämförbara kommuner, men lovhandläggningen går åt andra hållet — Linköping ligger klart under kommunmedianen på bygglov. Flaskhalsen sitter i planskedet, inte i lovgivningen. Kostnaden för fysisk planering per invånare ligger samtidigt under jämförelsekommunernas.',
  },
  {
    id: 'kostnad',
    status: 'kompletterad',
    kpi: ['N15009', 'N11020', 'N17008', 'N20060', 'N28033', 'N03187'],
    rubrik: 'Vad lokalerna kostar verksamheten',
    kommentar:
      'Kommunens bokförda lokalkostnad enligt Räkenskapssammandraget — hyresnotan ovan sedd från verksamhetens sida. Detta är kostnadsmått, inte betyg: en hög kostnad per elev kan spegla nyproduktionsandel, standard eller internhyresmodell lika gärna som ineffektivt lokalutnyttjande. Att interna köp för gemensamma lokaler är noll i Linköping visar just att bokföringen skiljer sig mellan kommuner.',
  },
  {
    id: 'beslut',
    status: 'kompletterad',
    kpi: ['N02884', 'N02842', 'N01926', 'N01953', 'N01957'],
    rubrik: 'Vad demografin säger om dimensioneringen',
    kommentar:
      'Beslutsstödet utgår från när lokalen behövs. SCB:s kommunframskrivningar via Kolada visar vad behovet väntas bli. Tioårshorisonten är den som motsvarar ledtiden för nybyggnad med ny detaljplan — det som beslutas idag levereras in i den kullen.',
  },
  {
    id: 'kallor',
    status: 'ingen',
    kpi: [],
    kommentar:
      'Källförteckning. Kolada ligger som två egna poster i källregistret, under området Kommunstatistik.',
  },
];

const STATUS_ETIKETT = {
  kompletterad: 'Kolada-kollad',
  delvis: 'Delvis kollad',
  ingen: 'Ingen Kolada-motsvarighet',
};
...
/**
 * Bygger panelen för en sektion som HTML-sträng.
 *
 * Strängbygge och inte JSX, eftersom panelen skjuts in i den befintliga
 * HTML-body som renderas i EN enda div. Att i stället dela upp body i flera
 * React-noder skulle bryta `.spar-section:nth-of-type(even)` och
 * `.spar-section + .spar-section` i sidans egen CSS.
 */
function byggPanel(sektion, kolada) {
  const flagga = `<span class="kk-flagga kk-flagga-${sektion.status}">${STATUS_ETIKETT[sektion.status]}</span>`;

  if (sektion.status === 'ingen') {
    return `<div class="kk-notis kk-status-ingen">${flagga}<p>${escapeHtml(sektion.kommentar)}</p></div>`;
  }

  const rader = sektion.kpi
    .map(id => {
      const matt = kolada.katalog.find(m => m.id === id);
      const v = kolada.varden[id];
      const b = kolada.bearbetat[id];
      if (!matt || !v) return '';
      const d = matt.decimaler;

      const cell = nyckel => {
        const ref = v.referenser[nyckel];
        if (!ref || ref.varde === null) {
          return `<td class="num"><span class="kk-saknas" title="${escapeHtml(ref ? ref.saknas || '' : '')}">–</span></td>`;
        }
        const stil = avvikelseStil(matt, b ? b.avvikelse[nyckel] : null);
        return `<td class="num">${formatSv(ref.varde, d)}<span class="kk-avvikelse is-${stil.ton}">${stil.text}</span></td>`;
      };

      const median = b && b.fordelning ? formatSv(b.fordelning.median, d) : '–';
      const percentil =
        b && b.percentil !== null
          ? `p${b.percentil}<span class="kk-n">av ${b.fordelning.n} kn</span>`
          : '<span class="kk-saknas" title="Percentil redovisas inte för absoluta tal, där den bara skulle mäta kommunstorlek">–</span>';

      return [
        '<tr>',
        `<th scope="row">${escapeHtml(matt.kort)}`,
        `<span class="kk-enhet">${escapeHtml(matt.enhet)}${matt.riktning === 'ingen' ? '' : ` · ${matt.riktning === 'lagre_battre' ? 'lägre är bättre' : 'högre är bättre'}`}</span>`,
        `<span class="kk-klass kk-klass-${matt.klass}">${matt.klass === 'styrmatt' ? 'Styrmått' : matt.klass === 'kostnadsmatt' ? 'Kostnadsmått' : 'Kontextmått'}</span>`,
        `<span class="kk-kpi">${matt.id}</span>`,
        '</th>',
        `<td class="num kk-td-lkpg">${formatSv(v.varde, d)}<span class="kk-ar">${v.ar}</span></td>`,
        cell('liknande'),
        cell('storre-stad'),
        cell('riket'),
        `<td class="num">${median}</td>`,
        `<td class="num">${percentil}</td>`,
        '</tr>',
      ].join('');
    })
    .join('');

  const harBearbetning = sektion.kpi.some(id => kolada.bearbetat[id] && kolada.bearbetat[id].percentil !== null);

  return [
    `<div class="kk-panel kk-status-${sektion.status}">`,
    `<div class="kk-panel-head">${flagga}<h3>${escapeHtml(sektion.rubrik || '')}</h3></div>`,
    `<p class="kk-kommentar">${escapeHtml(sektion.kommentar)}</p>`,
    '<div class="kk-tabell-wrap"><table class="kk-tabell"><thead><tr>',
    '<th scope="col">Nyckeltal</th>',
    '<th scope="col" class="num">Linköping</th>',
    '<th scope="col" class="num">Liknande kommuner</th>',
    '<th scope="col" class="num">Större stad</th>',
    '<th scope="col" class="num">Riket</th>',
    '<th scope="col" class="num">Kommunmedian</th>',
    '<th scope="col" class="num">Percentil</th>',
    `</tr></thead><tbody>${rader}</tbody></table></div>`,
    `<p class="kk-kalla">Källa: Kolada — ${sektion.kpi.join(', ')}. Hämtat ${escapeHtml(kolada.meta.hamtat)}.`,
    ' <a href="/kallregister#kolada-planledtider">Källkort</a>',
    harBearbetning
      ? '<br>Percentil, kommunmedian och avvikelse är egen bearbetning av data från Kolada och får inte tillskrivas Kolada.'
      : '',
    '</p>',
    '</div>',
  ].join('');
...
/** Sammanfattar täckningen, för bannern överst. */
function tackning() {
  return SEKTIONSSTATUS.reduce((ut, status) => {
    ut[status] = KOLLADE_SEKTIONER.filter(s => s.status === status).length;
    return ut;
  }, {});
}
