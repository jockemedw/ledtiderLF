...
import { slugify, anchorIdForElement, snapToCommentable, COMMENTABLE_SELECTOR } from '../anchor.js';
...
describe('snapToCommentable', () => {
  it('frågar closest med COMMENTABLE_SELECTOR och returnerar träffen', () => {
    const parent = { tagName: 'P' };
    const el = { closest: (sel) => (sel === COMMENTABLE_SELECTOR ? parent : null) };
    expect(snapToCommentable(el)).toBe(parent);
  });

  it('faller tillbaka till elementet självt när ingen kommenterbar förälder finns', () => {
    const el = { closest: () => null };
    expect(snapToCommentable(el)).toBe(el);
  });
});
