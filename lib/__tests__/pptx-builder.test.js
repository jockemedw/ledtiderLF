...
describe('slidesForSections', () => {
  ...
  it('rapporterar sektioner utan kurerad slide', () => {
    const { slideIds, missingSections } = slidesForSections(['hierarki', 'finns-inte']);
  ...
  it('varje kurerad slide pekar på en sektion som finns på sidan', () => {
    ...
    // 'dp-provning' ligger på /detaljplan och ingår bara i populärversionen —
    // den är medvetet inte valbar i plock-läget.
    const undantag = new Set(['dp-provning']);
    for (const s of CONFIG.slides) {
      if (!s.ankare || undantag.has(s.ankare)) continue;
...
describe('buildPptx', () => {
  ...
  it('utan ids byggs den kurerade populärversionen', async () => {
    ...
    expect(result.slideIds).toEqual(['titel', 'kostnad-80', 'hierarki', 'ledtider', 'provning']);
