...
describe('slidesForSections', () => {
  it('mappar sektion-id till slide-id via ankare', () => {
    ...
    expect(slideIds).toEqual([
      'oversikt', 'oversikt-hierarki',
      'kostnad-80', 'kostnad-kort', 'kostnad-omvarld',
    ]);
  ...
  it('bevarar sektionernas ordning i resultatet', () => {
    ...
    expect(a.slideIds[0]).toBe('kostnad-80');
    expect(b.slideIds[0]).toBe('oversikt');
    expect([...a.slideIds].sort()).toEqual([...b.slideIds].sort());
  });

  it('rapporterar sektioner utan kurerad slide', () => {
    const { slideIds, missingSections } = slidesForSections(['oversikt', 'finns-inte']);
    expect(slideIds).toEqual(['oversikt', 'oversikt-hierarki']);
...
describe('buildPptx', () => {
  ...
  it('utan ids byggs den kurerade populärversionen', async () => {
    ...
    expect(result.slideIds).toEqual([
      'titel', 'kostnad-80', 'oversikt', 'oversikt-hierarki', 'ledtider',
      'provning-inledning', 'provning', 'provning-moment', 'provning-moment-2',
      'provning-varfor',
    ]);
  ...
  it('sektionsexport får titelslide som omslag + en slide per sektion', async () => {
    ...
    expect(result.slideIds).toEqual([
      'titel', 'moduler', 'beslut', 'beslut-matris', 'beslut-forutsattning',
    ]);
