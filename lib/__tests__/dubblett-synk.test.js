...
// Samma siffror lever i osynkade lager: siffror.json och HTML-innehållet.
// Hårdkontrollen 2026-08-26 hittade flera glidningar ("44 mån", "7 850 m²",
// Vk5A) som uppstått just så. Tabellen nedan binder de mest bärande
// dubbletterna: ändras ett nyckeltalsvärde eller en HTML-förekomst utan att
// övriga lager följer med, larmar testet. Uppdatera då ALLA ställen och raden
// här. Pptx-exporten läser numera sin text ur lokalforsorjning.html
// (lib/sid-innehall.js) och är därför inget eget lager att synka.
...
const KRAV = [
  ...
  // kr-lss: HTML/pptx-dubbletterna (20–30 mkr) försvann med sektionen
  // "Konkreta exempel" — endast nyckeltalsvärdet låses numera.
  { nyckeltal: 'kr-lss', varde: '24 000–36 000 kr/m² BTA', forekomster: [] },
  // Siffran nämns bara i källraden — spår A:s text är medvetet hållen utan tal.
  { nyckeltal: 'kr-hyra-vs-aga-kommuninvest', varde: '+23–28 %', forekomster: [
    ['lokalforsorjning.html', '23–28 %', 1],
  ]},
  ...
  { etikett: 'Ledtidsindex-snittet 46 mån är utbytt mot Kolada-medianen i hela guiden', forekomster: [], saknas: [
    ['lokalforsorjning.html', '46 mån'],
  ]},
  // Planavsnittet, spår D:s fasrad och dess begränsningar ska ange samma median.
  { nyckeltal: 'planledtid-kommunmedian-antagande', varde: '29 mån', forekomster: [
    ['lokalforsorjning.html', '29 mån', 3],
  ]},
...
describe('dubblettsynk mellan siffror.json och HTML', () => {
  ...
  for (const krav of KRAV) {
    ...
    it(namn, () => {
      ...
      for (const [fil, strang] of krav.saknas || []) {
        expect(antal(filer[fil], strang), `'${strang}' ska inte längre finnas i ${fil}`).toBe(0);
      }
    });
  }
});
