...
describe('dataintegritet', () => {
  ...
  const hkKallorAktiv = register.kallor.some(k => k.hardkontroll);
  const hkNyckeltalAktiv = siffror.nyckeltal.some(n => n.hardkontroll);

  it.runIf(hkKallorAktiv)('varje källa har giltig hardkontroll', () => {
    for (const k of register.kallor) {
  ...
  it.runIf(hkNyckeltalAktiv)('varje nyckeltal har hardkontroll-datum och källkrav', () => {
    for (const n of siffror.nyckeltal) {
