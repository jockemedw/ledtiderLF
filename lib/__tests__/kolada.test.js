...
describe('Kolada — märkning mot Koladas egna beskrivningar', () => {
  // Ommärkningen 2026-09-04: materialet påstod att planmåtten avsåg
  // bostadsplaner och kom från Boverkets plan- och bygglovsenkät. Båda var
  // fel. Testerna binder därför märkningen till det Kolada självt skriver.
  const BOSTADSORD = /bostad|bostäder|bostads|flerbostadshus|boendeyta|lägenhet/i;

  it('sätter bostadsflaggan bara där Koladas beskrivning räknar bostäder', () => {
    for (const m of data.katalog.filter(x => x.tangerande)) {
      const txt = `${m.kolada_titel || ''} ${m.kolada_beskrivning || ''}`;
      expect(BOSTADSORD.test(txt), `${m.id} är flaggad som bostadsmått utan stöd i Koladas text`).toBe(
        true,
      );
    }
  });

  it('flaggar aldrig de fyra planmåtten som bostadsmått', () => {
    // Kolada formulerar frågan som "de detaljplaner som antogs i kommunen",
    // och för N07929 uttryckligen alla detaljplaner oavsett förfarande.
    for (const id of ['N07926', 'N07927', 'N07928', 'N07929']) {
      const m = data.katalog.find(x => x.id === id);
      expect(m.tangerande, `${id} har återfått bostadsflaggan`).toBeFalsy();
      expect(m.kolada_beskrivning, `${id} saknar Koladas beskrivning`).toBeTruthy();
      expect(BOSTADSORD.test(m.kolada_beskrivning), `${id}: Kolada har börjat skriva bostäder`).toBe(
        false,
      );
    }
  });

  it('bär Koladas egen källangivelse på varje mått', () => {
    for (const m of data.katalog) {
      expect(m.kolada_kalla, `${m.id} saknar källangivelse`).toBeTruthy();
    }
    // Källan varierar per mått, inte per grupp — planprocessgruppen har flera.
    const planKallor = new Set(
      data.katalog.filter(m => m.grupp === 'planprocess').map(m => m.kolada_kalla),
    );
    expect(planKallor.size, 'planprocessgruppen förutsätts ha flera källor').toBeGreaterThan(1);
  });

  it('tillskriver aldrig Kolada-måtten Boverket', () => {
    // Kolada anger SKR och kommunernas egen undersökning för N079xx.
    for (const m of data.katalog) {
      expect(m.kolada_kalla, `${m.id}`).not.toMatch(/Boverket/i);
      expect(m.not || '', `${m.id}: noten påstår Boverket`).not.toMatch(/Boverkets plan- och bygglovsenkät/);
    }
    for (const g of data.grupper) {
      expect(g.ingress, `gruppen ${g.id} påstår Boverket`).not.toMatch(/Boverket/);
    }
    const sida = fs.readFileSync(path.join(ROT, 'pages', 'linkoping.js'), 'utf-8');
    expect(sida, 'sidan påstår Boverket som källa').not.toMatch(/Boverket/);
  });

  it('håller etikett och enhet i linje med Koladas titel', () => {
    for (const m of data.katalog) {
      expect(m.kolada_titel, `${m.id} saknar Koladas titel`).toBeTruthy();
      expect(m.kort.length, `${m.id} har tom etikett`).toBeGreaterThan(2);
      expect(m.enhet.length, `${m.id} har tom enhet`).toBeGreaterThan(0);
    }
  });
});

describe('Kolada — mått som mäter ägarstruktur i stället för sak', () => {
  // Linköping äger sina verksamhetslokaler genom Lejonfastigheter. Kommunens
  // investeringsutgifter per verksamhet mäter därför var ägandet sitter, inte
  // vad som byggs: samma kommun och år låg på p9 för pedagogisk verksamhet och
  // p84 för kultur och fritid. N03187, interna köp för gemensamma lokaler, stod
  // på 0 kr/inv och percentil 0 av samma skäl — bolaget fakturerar externt.
  const BORTTAGNA = ['N03150', 'N03160', 'N03163', 'N03166', 'N03187'];

  it('håller de verksamhetsindelade kommunraderna utanför katalogen', () => {
    for (const id of BORTTAGNA) {
      expect(KATALOG.find(m => m.id === id), `${id} har återinförts`).toBeUndefined();
      expect(data.katalog.find(m => m.id === id), `${id} ligger kvar i data`).toBeUndefined();
      expect(data.varden[id], `${id} har värden kvar i data`).toBeUndefined();
    }
  });

  it('behåller ägarspegeln kommun mot koncern', () => {
    // Paren är hela poängen med avsnittet — det är där bolaget syns.
    for (const [kommun, koncern] of [['N03041', 'N03051'], ['N03012', 'N03063']]) {
      expect(data.varden[kommun], `${kommun} saknas`).toBeTruthy();
      expect(data.varden[koncern], `${koncern} saknas`).toBeTruthy();
    }
  });
});
